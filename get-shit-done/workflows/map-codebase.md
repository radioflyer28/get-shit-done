...
**How to detect:** Check if you have access to an `Agent` tool (may be capitalized as `Agent` or lowercase as `agent` depending on runtime), or an equivalent subagent tool provided by the active runtime adapter. If you do NOT have an `Agent`/`agent`/equivalent subagent tool (or only have tools like `browser_subagent` which is for web browsing, NOT code analysis):
...
<step name="sequential_mapping" condition="Subagent delegation is NOT available (e.g. Antigravity, Gemini CLI)">
When subagent delegation is unavailable, perform codebase mapping sequentially in the current context. This replaces `spawn_agents` and `collect_confirmations`.
...
- If subagent delegation is NOT available: 4 sequential mapping passes performed inline (never using browser_subagent)
