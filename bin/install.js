...
function convertClaudeCommandToPiSkill(content, skillName, _runtime = 'pi', cmdNames = null) {
  const skill = convertClaudeCommandToClaudeSkill(content, skillName, 'pi', cmdNames);
  return injectPiSubagentsSkillAdapter(convertClaudeToPiContent(skill));
}
...
function _applyRuntimeRewrites(content, runtime, pathPrefix) {
  ...
  switch (runtime) {
    ...
    case 'pi':
      ...
      content = content.replace(/~\/\.pi\/agent\b/g, normalizedPathPrefix);
      content = content.replace(/\$HOME\/\.pi\/agent\b/g, normalizedPathPrefix);
      content = processAttribution(content, getCommitAttribution(runtime));
...
function _runLegacyInstallMigrations(runtime, configDir, scope = 'global') {
  ...
  if (runtime === 'claude' || runtime === 'qwen' || runtime === 'hermes' || runtime === 'pi') {
...
function _runLegacyUninstallCleanup(runtime, configDir, scope = 'global') {
  ...
  // Claude-local commands/gsd/ is the primary current location — skip here.
  const isLegacyCommandsGsd = runtime === 'qwen' || runtime === 'hermes' || runtime === 'pi' || (runtime === 'claude' && scope === 'global');
...
function install(isGlobal, runtime = 'claude', options = {}) {
  ...
  if (!isCodex && !isCopilot && !isCursor && !isWindsurf && !isTrae && !isCline && !isPi && fs.existsSync(hooksLibSrc)) {
