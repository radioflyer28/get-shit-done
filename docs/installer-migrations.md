...
Pi, Qwen Code, Trae, and Windsurf. The installer invokes the same migration runner
...
| Pi | Agent Skills standard directories in `skills/gsd-*/SKILL.md`; agents in `agents/` for pi-subagents users | Global `PI_AGENT_HOME`, `PI_CONFIG_DIR`, or `~/.pi/agent`; local `./.pi` | GSD owns generated skills, generated agents, and engine files only; Pi settings are left untouched | [Pi skills](https://pi.dev/docs/latest/skills), [Pi settings](https://pi.dev/docs/latest/settings); docs checked 2026-05-13 |
