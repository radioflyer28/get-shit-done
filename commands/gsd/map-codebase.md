...
argument-hint: "[--parallel] [--fast [--focus tech|arch|quality|concerns]] [--query <term>|status|diff|refresh] [area]"
allowed-tools:
  - Read
  - Bash
  - Glob
...
- **--parallel**: Explicitly authorizes Codex to use `spawn_agent` / `wait_agent` for mapper fan-out when those tools are available. Without this flag, Codex uses sequential mapping.
